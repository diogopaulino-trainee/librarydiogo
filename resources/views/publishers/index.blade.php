    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-end mb-4">
                <a href="{{ route('publishers.create') }}" class="btn btn-primary bg-blue-500 text-white hover:bg-blue-700 transition duration-300 shadow-md">
            ...
            <div class="max-full mx-auto mt-8 p-8 bg-white shadow-md rounded-lg border border-blue-500">
                <div class="flex justify-between items-center mb-4">
                    <form action="{{ route('publishers.index') }}" method="GET" class="flex items-center space-x-2">
                        <div class="relative">
                            <input type="text" name="search" placeholder="Search by name" value="{{ request('search') }}" class="input input-bordered input-primary w-64 pl-10" />
                    ...
                    <div class="flex space-x-2">
                        <a href="{{ route('publishers.index', ['sort_by' => 'name', 'order' => 'asc']) }}" class="btn btn-sm btn-outline">Sort A-Z</a>
                        <a href="{{ route('publishers.index', ['sort_by' => 'name', 'order' => 'desc']) }}" class="btn btn-sm btn-outline">Sort Z-A</a>
                        <a href="{{ route('publishers.index', ['sort_by' => 'created_at', 'order' => 'asc']) }}" class="btn btn-sm btn-outline">Oldest First</a>
                        <a href="{{ route('publishers.index', ['sort_by' => 'created_at', 'order' => 'desc']) }}" class="btn btn-sm btn-outline">Newest First</a>
                        <a href="{{ route('publishers.index', ['sort_by' => 'updated_at', 'order' => 'asc']) }}" class="btn btn-sm btn-outline">Least Updated</a>
                        <a href="{{ route('publishers.index', ['sort_by' => 'updated_at', 'order' => 'desc']) }}" class="btn btn-sm btn-outline">Recently Updated</a>
                    </div>
                </div>
                ...
                <table class="min-w-full bg-white border border-blue-500 shadow-md rounded-lg">
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-blue-500 hover:text-white group">
                            <td class="px-4 py-2 border-b">{{ $publisher->name }}</td>
                            <td class="px-4 py-2 border-b text-center whitespace-nowrap space-x-2">
                                <a href="{{ route('publishers.show', $publisher) }}" title="View more details" class="text-blue-500 group-hover:text-white transition duration-200">
                                ...
                                <button onclick="openModal('modal-{{ $publisher->id }}')" title="View Timestamps" class="text-gray-500 group-hover:text-white transition duration-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10m-10 4h10m-9 4h8" />
                                    </svg>
                                </button>
                                <a href="{{ route('publishers.edit', $publisher) }}" class="text-yellow-400 hover:text-yellow-800">Edit</a>
                                <a href="{{ route('publishers.delete', $publisher) }}" class="text-red-600 hover:text-red-800">Delete</a>
                            </td>
                        </tr>
                        <div id="modal-{{ $publisher->id }}" class="fixed inset-0 bg-blue-900 bg-opacity-55 hidden items-center justify-center z-50">
                            <div class="bg-white rounded-lg shadow-md border border-blue-200 max-w-sm w-full p-6 relative">
                                ...
                                <div class="flex justify-end mt-4">
                                    <button onclick="closeModal('modal-{{ $publisher->id }}')" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-700">Close</button>
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                <div class="flex justify-end mt-6">
                    <a href="{{ route('publishers.export') }}" 
